...
accuracy = tf.reduce_mean(tf.cast(tf.equal(predited, Y), dtype=tf.float32))
...
with tf.Session() as sess:
    ...
    for i in range(10001):
        ...
        if i % 1000 == 0:
            print(i, sess.run([cost, d_W1], feed_dict={X:x_data,Y:y_data}),
                  sess.run([W1,W2]))
            h, c, a = sess.run([Y_pred, predicted, accuracy], feed_dcit={X:x_data,Y:y_data})
            print("hypothesis:", h, "\n correct:",c, "\nAccuraycy:",a)
